...
public class ServicioClientes {
	...
	public Cliente buscar() {
		Cliente cliente = new Cliente("1727392035","Brayan Narváez",0);
		return cliente;
	}
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response bsucarPorCedula(@PathParam("cedulaParam")String cedula){
		ClientesBDD cli = new ClientesBDD();
	...
	@Path("buscarPorNH/{numeroHijos}")
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response bsucarPorNH(@PathParam("numeroHijos")int numeroHijos){
		ClientesBDD cli = new ClientesBDD();
		Cliente cliente = null;
		try {
			cliente = cli.buscarPorNH(numeroHijos);
			return Response.ok(cliente).build();
		} catch (KrakeDevException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
