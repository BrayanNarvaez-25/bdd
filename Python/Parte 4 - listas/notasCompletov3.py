...
from utilNotas import buscarEstudiante,calcularTotal
#VARIABLES GLOBALES
estudiantes=[
...
def fnGuardar():
    ...
    faltas = fnObtenerEntero(txtInasistencias,lblErrorInasistencias,0)
    if estEncontrado == None: #no existe en la lista
        if (nota1 != None and nota2 != None and nota3 != None and nota1 >= 0 and nota1 <= 10 and nota2 >= 0 and nota2 <= 10 and nota3 >= 0 and nota3 <= 10) and (nombre != None and apellido != None and cedula != None) and (faltas !=None and faltas >= 0):
            total = calcularTotal(nota1,nota2,nota3,faltas)
            lblTotal.config(text=total)
            estudiantes.append(f"{cedula}#{nombre}#{apellido}#{nota1}#{nota2}#{nota3}#{faltas}#{total}")
